# novo_bd1971_robusto/scraping/scraping_jogadores_treinadores_estadios_v4.py
import requests
...
class OGolScraperRelacional:
    def __init__(self, url_lista):
        ...
        # base delay (seconds) between requests; adaptive jitter applied
        self.delay_base = 3
        # max consecutive pages with empty content before abort
        ...
        # Map auxiliar por nome+nascimento -> chave na jogadores_dict (para compatibilidade)
        self.jogadores_por_nome_nascimento = {}
        ...
        # Flag se jogadores foram atualizados (para reescrever CSV)
        self.jogadores_updated = False
        ...
        # contador de páginas vazias consecutivas (possível bloqueio)
        self.empty_page_counter = 0
    ...
    def _carregar_ids_existentes(self):
        ...
        if os.path.exists(path_jogadores):
            with open(path_jogadores, 'r', encoding='utf-8') as f:
                reader = csv.DictReader(f)
                for row in reader:
                    apelido = row.get('apelido', '').strip() if 'apelido' in row else ''
                    chave = f"{row['nome']}_{row.get('nascimento', '')}_{apelido}"
                    registro = {
                        'id': int(row['id']),
                        'nome': row['nome'],
                        'apelido': apelido,
                    ...
                    self.jogadores_dict[chave] = registro
                    key_name_birth = f"{row['nome']}_{row.get('nascimento', '')}"
                    # preferir chave com apelido se presente
                    self.jogadores_por_nome_nascimento[key_name_birth] = chave
    ...
    def _get_soup(self, url):
        """Faz a requisição HTTP com tratamento de erros e detecção de bloqueio."""
        tentativa = 0
        max_tentativas = 5
        ...
        while tentativa < max_tentativas:
            try:
                ...
                # Detecta 429 explicitamente e aplica backoff
                ...
                text = r.text
                # heurística simples para detectar bloqueio ou páginas vazias
                if len(text) < 800 or re.search(r'captcha|acesso negado|access denied|blocked|permission denied|robot', text, re.I):
                    self.empty_page_counter += 1
                    print(f"⚠️ Conteúdo suspeito/pequeno ({len(text)} chars). empty_page_counter={self.empty_page_counter}")
                    if self.empty_page_counter >= self.max_empty_pages:
                        raise Exception("❌ Possível bloqueio detectado: muitas páginas vazias ou CAPTCHA. Parando execução.")
                    # aguarda e tenta novamente com backoff
                    tentativa += 1
                    espera = delay * (tentativa + 1)
                    time.sleep(espera + random.uniform(0, 2))
                    continue

                # conteúdo ok — reset counter
                self.empty_page_counter = 0
    ...
    def _get_ou_criar_local(self, cidade_completa):
        ...
        if "(" in cidade_completa and ")" in cidade_completa:
            ...
            partes = cidade_completa.rsplit("-", 1)
    ...
    def processar_clube(self, url_clube):
        ...
        if not container:
            self.empty_page_counter += 1
            print("⚠️ Container de clube não encontrado")
            if self.empty_page_counter >= self.max_empty_pages:
                raise Exception("❌ Possível bloqueio detectado ao processar clube. Parando execução.")
            return None
    ...
    def processar_estadio(self, url_estadio):
        ...
        if not container:
            self.empty_page_counter += 1
            print("⚠️ Container de estádio não encontrado")
            if self.empty_page_counter >= self.max_empty_pages:
                raise Exception("❌ Possível bloqueio detectado ao processar estádio. Parando execução.")
            return None
    ...
    def processar_jogador(self, url_jogador, apelido_from_list=None):
        """Processa jogador usando chave de atributos (nome + nascimento + apelido)
           apelido_from_list: apelido extraído do texto do link na página da partida
        """
        if not url_jogador:
            return None
        ...
        if not container:
            self.empty_page_counter += 1
            print("⚠️ Container de biografia não encontrado para jogador")
            if self.empty_page_counter >= self.max_empty_pages:
                raise Exception("❌ Possível bloqueio detectado ao processar jogador. Parando execução.")
            return None
        ...
        for div in container.find_all("div", class_=["bio", "bio_half"]):
            ...
            if "Nome" in campo and "nome" not in dados:
                ...
                if "Falecido" in (valor or ""):
                    dados["falecimento"] = valor
                    dados["aposentado"] = 1
                elif "Aposentado" in (valor or ""):
                    dados["aposentado"] = 1

        # Se o apelido veio da página de lista (link), use-o como candidato
        apelido_candidato = apelido_from_list.strip() if apelido_from_list else None

        chave_atributos = f"{dados.get('nome', '')}_{dados.get('nascimento', '')}_{apelido_candidato or ''}"

        # Verifica chave exata (com apelido)
        if chave_atributos in self.jogadores_dict:
            jogador_id = self.jogadores_dict[chave_atributos]['id']
            print(f"   ✓ Jogador já existente (full key): {dados.get('nome', '')} (ID: {jogador_id})")
            self.url_cache['jogadores'][url_jogador] = jogador_id
            return jogador_id

        # Verifica por nome+nascimento (compatibilidade)
        key_name_birth = f"{dados.get('nome', '')}_{dados.get('nascimento', '')}"
        if key_name_birth in self.jogadores_por_nome_nascimento:
            existing_key = self.jogadores_por_nome_nascimento[key_name_birth]
            registro_existente = self.jogadores_dict.get(existing_key)
            if registro_existente:
                jogador_id = registro_existente['id']
                # Se registro existente não tem apelido e temos um candidato, atualiza
                if (not registro_existente.get('apelido')) and apelido_candidato:
                    registro_existente['apelido'] = apelido_candidato
                    # atualizar mapeamento para nova chave com apelido
                    nova_chave = f"{registro_existente['nome']}_{registro_existente['nascimento']}_{apelido_candidato}"
                    self.jogadores_dict[nova_chave] = registro_existente
                    del self.jogadores_dict[existing_key]
                    self.jogadores_por_nome_nascimento[key_name_birth] = nova_chave
                    self.jogadores_updated = True
                    print(f"   ➤ Apelido '{apelido_candidato}' preenchido para jogador ID {jogador_id}")
                print(f"   ✓ Jogador já existente por nome/data: {dados.get('nome', '')} (ID: {jogador_id})")
                self.url_cache['jogadores'][url_jogador] = jogador_id
                return jogador_id
        ...
        chave_final = f"{registro['nome']}_{registro['nascimento']}_{registro['apelido']}"
        self.jogadores_dict[chave_final] = registro
        self.jogadores_por_nome_nascimento[f"{registro['nome']}_{registro['nascimento']}"] = chave_final
        self.url_cache['jogadores'][url_jogador] = jogador_id
    ...
    def processar_treinador(self, url_treinador):
        ...
        if not container:
            self.empty_page_counter += 1
            print("⚠️ Container de treinador não encontrado")
            if self.empty_page_counter >= self.max_empty_pages:
                raise Exception("❌ Possível bloqueio detectado ao processar treinador. Parando execução.")
            return None
    ...
    def processar_arbitro(self, url_arbitro):
        ...
        if not container:
            self.empty_page_counter += 1
            print("⚠️ Container de árbitro não encontrado")
            if self.empty_page_counter >= self.max_empty_pages:
                raise Exception("❌ Possível bloqueio detectado ao processar árbitro. Parando execução.")
            return None
    ...
    def processar_detalhes_partida(self, url_partida, partida_id, mandante_id, visitante_id):
        ...
        for idx, coluna in enumerate(colunas):
            ...
            for player_div in coluna.find_all("div", class_="player"):
                ...
                link_href = urljoin(self.base_url, link_tag["href"])
                link_text = link_tag.get_text(strip=True)

                try:
                    jogador_id = self.processar_jogador(link_href, apelido_from_list=link_text)
                except Exception as e:
    ...
    def salvar_csvs(self):
        ...
        # Reescreve jogadores.csv se houver alterações (novos ou atualizações de apelido)
        if self._novo_jogador or self.jogadores_updated:
            ...
            # ordenar por id para estabilidade
            todos = sorted(self.jogadores_dict.values(), key=lambda x: x['id'])
    ...
    def executar(self, edicao_id=1):
        ...
        for linha in tabela.find_all("tr"):
            ...
            # aguarda um pouco com jitter para reduzir chances de bloqueio
            time.sleep(self.delay_base + random.uniform(0, 2))
