...
class OGolScraperRelacional:
    ...
    def _carregar_existentes(self):
        """
        Se existirem CSVs, carrega IDs máximos e popula dicionários por url (quando as tabelas tiverem coluna 'url').
        Isso permite continuar a contagem de IDs e evitar re-downloads.
        """
        def try_load_csv(path, key_by_url=False):
            ...
            with open(path, "r", encoding="utf-8") as f:
                ...
                rows = list(reader)
            if key_by_url:
                # retorna header e rows
                return rows
            return rows

        # Jogadores (espera-se que jogadores.csv possa ter coluna 'url')
        path_jog = os.path.join(self.output_dir, "jogadores.csv")
        if os.path.exists(path_jog):
            with open(path_jog, "r", encoding="utf-8") as f:
                reader = csv.DictReader(f)
                max_id = 0
                for r in reader:
                    try:
                        ...
                        pass
                self.next_jogador_id = max(self.next_jogador_id, max_id + 1)
            print(f"   ➤ jogadores.csv lido, next_jogador_id={self.next_jogador_id}")

        # Treinadores
        path_tr = os.path.join(self.output_dir, "treinadores.csv")
        if os.path.exists(path_tr):
            with open(path_tr, "r", encoding="utf-8") as f:
                ...
                for r in reader:
                    try:
                        ...
                        url = r.get('url')
                        if url:
                            self.treinadores_dict[url] = {'id': iid, 'nome': r.get('nome',''), 'url': url}
                    except:
                        pass
                self.next_treinador_id = max(self.next_treinador_id, max_id + 1)
            print(f"   ➤ treinadores.csv lido, next_treinador_id={self.next_treinador_id}")

        # Árbitros
        path_ar = os.path.join(self.output_dir, "arbitros.csv")
            ...
            with open(path_ar, "r", encoding="utf-8") as f:
                ...
                for r in reader:
                    try:
                        ...
                        url = r.get('url')
                        if url:
                            self.arbitros_dict[url] = {'id': iid, 'nome': r.get('nome',''), 'url': url}
                    except:
                        pass
                self.next_arbitro_id = max(self.next_arbitro_id, max_id + 1)
            print(f"   ➤ arbitros.csv lido, next_arbitro_id={self.next_arbitro_id}")

        # Clubes
        path_cl = os.path.join(self.output_dir, "clubes.csv")
            ...
            with open(path_cl, "r", encoding="utf-8") as f:
                ...
                for r in reader:
                    try:
                        ...
                        pass
                self.next_clube_id = max(self.next_clube_id, max_id + 1)
            print(f"   ➤ clubes.csv lido, next_clube_id={self.next_clube_id}")

        # Estádios
        path_es = os.path.join(self.output_dir, "estadios.csv")
            ...
            with open(path_es, "r", encoding="utf-8") as f:
                ...
                for r in reader:
                    try:
                        ...
                        pass
                self.next_estadio_id = max(self.next_estadio_id, max_id + 1)
            print(f"   ➤ estadios.csv lido, next_estadio_id={self.next_estadio_id}")

        # Partidas: recuperar max id e, se existir, marcar partidas já registradas por URL (evita duplicar partidas)
        path_pa = os.path.join(self.output_dir, "partidas.csv")
        if os.path.exists(path_pa):
            with open(path_pa, "r", encoding="utf-8") as f:
                reader = csv.DictReader(f)
                max_id = 0
                partidas_urls = set()
                for r in reader:
                    try:
                        ...
                        url = r.get('url')
                        if url:
                            partidas_urls.add(url)
                    except:
                        pass
                self.next_partida_id = max(self.next_partida_id, max_id + 1)
            # guardamos URLs para evitar re-criar partida se já existir
            self._partidas_existentes_urls = partidas_urls
            print(f"   ➤ partidas.csv lido, next_partida_id={self.next_partida_id}, partidas_existentes={len(partidas_urls)}")
        else:
            self._partidas_existentes_urls = set()

        # Eventos: recuperar next_evento_id
        path_ev = os.path.join(self.output_dir, "eventos_partida.csv")
        if os.path.exists(path_ev):
            with open(path_ev, "r", encoding="utf-8") as f:
                reader = csv.DictReader(f)
                max_id = 0
                for r in reader:
                    try:
                        ...
                        pass
                self.next_evento_id = max(self.next_evento_id, max_id + 1)
            print(f"   ➤ eventos_partida.csv lido, next_evento_id={self.next_evento_id}")
